...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        scrollView.contentSize.height = 1080
        imageView.frame = CGRect(x: 0, y: 0, width:frameWidth, height:500) //setting width and height for image
        imageView.contentMode = .ScaleAspectFill // fixing aspect
        scrollView.addSubview(imageView)
        
        let frameForButton = CGRect(x:(frameWidth*(0.34)), y: 280, width:100, height: 40)
        let buttonForMental = UIButton(frame: frameForButton)
        //  buttonForMental.titleLabel?.text = "Testing"
        //  buttonForMental.backgroundColor = UIColor.redColor() // to test location of button on UI
       ...
       // secondImageView.frame = CGRect(x: 0, y: 500, width: frameWidth, height:500)
        ...
        let frameForMentalIconButton = CGRect(x: (frameWidth * 0.45), y: 945, width: 30, height: 30)
       ...
       // mentalIconButton.backgroundColor = UIColor.redColor()
        ...
        let frameForLogMentalButton = CGRect(x: (frameWidth * 0.45), y: 420, width: 30, height: 35)
